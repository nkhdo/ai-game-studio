...
<template>
  <section class="card movement-step accordion-item" :class="{ 'is-open': studio.activePanel === 'movement' }">
    <button class="accordion-trigger" type="button" :aria-expanded="studio.activePanel === 'movement'" @click="studio.actions.setPanel('movement')"><span>Generate Movement</span><span class="accordion-trigger__icon" /></button>
    <div class="panel-body">
      ...
      <div class="field"><label class="field__label" for="motion-model">Model</label><select id="motion-model" v-model="studio.state.draft.motionModel" class="select"><option v-for="option in studio.videoModels" :key="option.id" :value="option.id">{{ option.label }}</option></select></div>
