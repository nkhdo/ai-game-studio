    ...
    <div class="panel-body"><div class="animation-workspace">
      ...
      <div class="animation-edit-pane">
        ...
        <div class="animation-editor">
          ...
          <div class="gif-preview quick-preview"><div class="quick-preview__stage"><img v-if="frames.length" :src="frames[position] ?? frames[0]" alt="Quick Animation preview" :style="{ transform: `scale(${zoom})` }" /><span v-else class="gif-preview__placeholder">Select frames to preview immediately</span></div><div class="quick-preview__overlay"><span>{{ frames.length ? position + 1 : 0 }} / {{ frames.length }}</span><button class="quick-preview__action" type="button" aria-label="Zoom out" :disabled="zoom <= .5" @click="zoom = Math.max(.5, zoom - .25)">−</button><button class="quick-preview__action" type="button" @click="zoom = 1">{{ Math.round(zoom * 100) }}%</button><button class="quick-preview__action" type="button" aria-label="Zoom in" :disabled="zoom >= 4" @click="zoom = Math.min(4, zoom + .25)">+</button><button class="quick-preview__action" type="button" @click="playing = !playing">{{ playing ? "Pause" : "Play" }}</button></div></div>
